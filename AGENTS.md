...
- Déploiement cible : [DEPLOY_COOLIFY.md](DEPLOY_COOLIFY.md), [PRODUCTION_CHECKLIST_COOLIFY.md](PRODUCTION_CHECKLIST_COOLIFY.md), [DEPLOYMENT_GUIDE.md](DEPLOYMENT_GUIDE.md), [Dockerfile](Dockerfile), [docker-compose.coolify.yml](docker-compose.coolify.yml).
- Rollback Render pendant migration : [DEPLOY.md](DEPLOY.md), [PRODUCTION_CHECKLIST.md](PRODUCTION_CHECKLIST.md), [render.yaml](render.yaml).
...
## Architecture
- Backend : Django 5.2 LTS, Python 3.13, DRF/JWT, Celery/Redis, WeasyPrint, Stripe, Hostinger SMTP, stockage Cloudinary ou S3 compatible en production.
...
- Configuration Django : [gestion_eebc/settings/base.py](gestion_eebc/settings/base.py), `dev.py`, `test.py`, `build.py`, `prod.py`. Les tests utilisent [gestion_eebc/settings/test.py](gestion_eebc/settings/test.py).
- Production cible : image Docker unique réutilisée par le web et le worker ; PostgreSQL et Redis restent des ressources Coolify séparées afin de découpler le cycle de vie applicatif et le cycle de vie des données.
...
- Build production : `docker build -t gestion-eebc .`.
...
- En test, Celery est eager, l'email est en mémoire et le stockage média est temporaire; cela ne prouve pas le comportement Redis, SMTP, Cloudinary/S3 ou Coolify.
...
- Ne pas réintroduire une dépendance Render dans `settings.prod`, `start.sh` ou la configuration média ; les fichiers Render sont conservés uniquement pour rollback pendant la migration.
- Ne pas activer Celery Beat en production Coolify tant que l'ensemble du schedule historique n'a pas été revu, notamment les tâches de backup local.
...
- Pour les documents/imports/uploads, vérifier taille/type de fichier, visibilité par rôle, chemins de stockage, Cloudinary/S3 et erreurs utilisateur.
...
- Pour la production Coolify, vérifier en plus : build Docker réel, migration one-shot, healthchecks, réseau privé vers PostgreSQL/Redis, proxy Traefik, persistance média, sauvegarde PostgreSQL distante et restauration testée.
