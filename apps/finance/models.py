...
- Le système de budget des groupes/départements
...
class FinancialTransaction(models.Model):
    ...
    # Lien avec le budget (pour les dépenses)
...
class Budget(models.Model):
    ...
    def spent_amount(self):
        ...
        from decimal import Decimal
        total = sum(item.spent_amount for item in self.items.all())
    ...
    def utilization_percentage(self):
        ...
        if self.total_approved > 0:
            return float((self.spent_amount / self.total_approved) * 100)
        return 0.0
...
class BudgetItem(models.Model):
    ...
    class ApprovalStatus(models.TextChoices):
        ...
        APPROVED = 'approved', 'Approuvé'
        REJECTED = 'rejected', 'Rejeté'
        PARTIALLY_APPROVED = 'partially_approved', 'Partiellement approuvé'
    ...
    approval_comments = models.TextField(
        blank=True,
        verbose_name="Commentaires d'approbation"
    )
    ...
    def utilization_percentage(self):
        ...
        if self.approved_amount > 0:
            return float((self.spent_amount / self.approved_amount) * 100)
        return 0.0
    
    @property
    def approval_percentage(self):
        """Calcule le pourcentage d'approbation par rapport à la demande."""
