...
def handle_announcement_multichannel(sender, instance, created, **kwargs):
    ...
    def dispatch():
        ...
        try:
            ...
            logger.exception(
                "Celery unavailable for announcement %s; using synchronous "
                "fallback",
                announcement_id,
            )
...
def handle_life_event_notification(sender, instance, created, **kwargs):
    ...
    pastors = User.objects.filter(
        role__icontains="pasteur", is_active=True
    ...
    if instance.event_type == "naissance":
        for pastor in pastors:
            Notification.objects.create(
                ...
                message=(
                    "Une naissance a été enregistrée pour "
                    f"{instance.primary_member.full_name}. Pensez à l'annoncer "
                    "lors du prochain culte."
                ),
        ...
        for pastor in pastors:
            Notification.objects.create(
                ...
                message=(
                    "Un décès a été enregistré concernant "
                    f"{instance.primary_member.full_name}. Une visite pastorale "
                    "est recommandée."
                ),
        ...
        if instance.requires_visit:
            main_pastor = User.objects.filter(
        ...
        if instance.requires_visit:
            ...
            VisitationLog.objects.create(
                ...
                summary=(
                    "Visite hospitalière : "
                    f"{instance.description or instance.title}"
                ),
            )
        ...
        for pastor in pastors:
            Notification.objects.create(
                ...
                message=(
                    "Un baptême a été enregistré pour "
                    f"{instance.primary_member.full_name}."
                ),
                notification_type="success",
                action_url=f"/admin/members/lifeevent/{instance.pk}/change/",
            )
